...
random.seed(30)	                        #zapis do pliku bitów z prawdopodobienstwem błędu
endFile = open('wynik.txt', 'w')
for x in range(0, len(bits)):
    r = random.randint(0, 100)
    if (r < fault_prob):                #prawdopodobieństwo wystąpienia błędu (25%)
        if (bits[x] == 1):
            endFile.write('0')
        else:
            endFile.write('1')
...
print("Procent nieprawidlowo przeslanych bitow: %.3f%%" %(countErrors(bits)*100))
